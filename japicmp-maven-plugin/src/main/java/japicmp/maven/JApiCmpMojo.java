...
public class JApiCmpMojo extends AbstractMojo {
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		List<JsfComponent> jsfComponents = compareJsfComponents(newVersionFile, oldVersionFile, jApiClasses);
		if (projectBuildDir != null && projectBuildDir.exists()) {
			try {
				...
				generateXmlOutput(newVersionFile, oldVersionFile, jApiClasses, jsfComponents, jApiCmpBuildDir, options);
	...
	private List<JsfComponent> compareJsfComponents(File newVersionFile, File oldVersionFile, List<JApiClass> jApiClasses) {
		JsfArchiveComparator jsfArchiveComparator = new JsfArchiveComparator();
		return jsfArchiveComparator.compare(oldVersionFile, newVersionFile, jApiClasses);
	}
	...
	private void generateXmlOutput(File newVersionFile, File oldVersionFile, List<JApiClass> jApiClasses, List<JsfComponent> jsfComponents, File jApiCmpBuildDir, Options options) throws IOException {
